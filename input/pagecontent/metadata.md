...
This page describes the computable metadata of the Meta module. The metadata
...
[CRMI STU2](https://hl7.org/fhir/uv/crmi/STU2/en/). It declares the following
metadata:

| Metadata | Value or use |
| --- | --- |
...
The IG-level `artifact-versionPolicy` and the module-specific codeable licence
extension are not declared. The guide-level licence remains `CC-BY-4.0`, and
...
expansion uses the
[Meta expansion manifest](Parameters-mii-param-meta-manifest.html), linked from
the ImplementationGuide through `cqf-expansionParameters` and configured by
`path-expansion-params` and `pin-manifest`.
...
[downloadable FHIR package](downloads.html). The human-readable
[Versioning](version-history.html) page explains the release version scheme.
