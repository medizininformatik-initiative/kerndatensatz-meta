...
| C | extension intro, SearchParameter-profile intro, `changes`, `index`, `search-parameters`, `translationinfo` | 6 paired machine-translation/language-status blocks: review both languages and remove the marker |
...
| [`Liste-Kerndatensatz-Suchparameter/Index.page.md`](https://simplifier.net/guide/mii-ig-modul-meta-2025/MII-IG-Meta/Technische-Implementierung/Liste-Kerndatensatz-Suchparameter) | [`search-parameters.md`](https://medizininformatik-initiative.github.io/kerndatensatz-meta/branches/migration/2026.0.0-template-v0.13.1/search-parameters.html) | family index page and 170-row table | 2 |
...
**Rendering replacements:** Simplifier FQL and artifact views are replaced by Publisher artifact pages and a static 170-row SearchParameter index derived from the preserved resources. No normative resource data is discarded.  
**M9 optional pages:** removed at artifact count 0: Operations, Value Sets, Researcher Guidance. Kept: Search Parameters (170), Extensions (1), Code Systems (1), Metadata Overview (user requested).  
...
| `C4-767a0b` | source prose survives | SearchParameter list | linked sentence is present, but URL normalization prevents exact run match | REV-1 |
...
| three C7 rows | migration-written content is marked | CodeSystem intro, SearchParameter page, index | Gate approval deliberately removed their review markers; the approved prose remains | accepted at Gates B/C |
...
**Verifier changes:** none. Runs: initial 65/12/19 → log/identity remediation 72/8/16 → post-approval marker clearance 71/11/16.
...
- [x] **search-parameters:** review two machine-translation/table-description blocks.
